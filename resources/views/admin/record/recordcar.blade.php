...
@section('title', 'Record Cash Advance Request (CAR)')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm transition-colors">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-sky-600 dark:text-sky-400 uppercase tracking-wider mb-1">
                ...
                <span>Financial & Operational Expenditure System</span>
            </div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <i class="fa-solid fa-receipt text-sky-600 dark:text-sky-400"></i>
                Record Cash Advance Request (CAR)
            </h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                Log audit pengajuan dana kasbon, belanja operasional lapangan, dan verifikasi persetujuan berjenjang.
            </p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- TOMBOL EXPORT CSV --}}
            <button type="button" onclick="exportCsvCar()" 
    ...
    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm transition-colors">
        <form method="GET" action="{{ route('admin.record.car') }}" id="filterFormCar" class="space-y-4">
            ...
            <div class="pt-2">
                <div class="relative w-full">
                    ...
                    <input type="text" id="table-search" placeholder="Pencarian cepat nomor CAR, nama pemohon, NIP, nama barang, rekening, atau alasan..." 
    ...
    {{-- CARDS STATISTIK SUMMARY HRMS / FINANSIAL CAR --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {{-- Card 1: Total Pengajuan CAR --}}
        <div class="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm flex items-center justify-between transition-colors">
            <div>
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Pengajuan CAR</p>
                <h3 class="text-2xl font-black text-slate-800 dark:text-slate-100 mt-1">
                    {{ number_format($metrics['total']) }}
                </h3>
                <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Dokumen terdaftar</p>
            </div>
        ...
        {{-- Card 2: Total Dana Disetujui (Rp) --}}
        <div class="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-emerald-100 dark:border-emerald-900/50 shadow-sm flex items-center justify-between transition-colors">
            <div>
                <p class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Total Dana Disetujui</p>
                <h3 class="text-xl font-black text-emerald-700 dark:text-emerald-400 mt-1">
                    Rp {{ number_format($metrics['total_dana_disetujui'], 0, ',', '.') }}
                </h3>
                <p class="text-[11px] text-emerald-600/80 dark:text-emerald-400/80 mt-0.5">{{ $metrics['approved'] }} Dokumen Selesai</p>
            </div>
        ...
        {{-- Card 3: CAR Menunggu Persetujuan --}}
        <div class="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-amber-100 dark:border-amber-900/50 shadow-sm flex items-center justify-between transition-colors">
            <div>
                ...
                <p class="text-[11px] text-amber-600/80 dark:text-amber-400/80 mt-0.5">SPV / Manager Approval</p>
            </div>
        ...
        {{-- Card 4: CAR Selesai / Disetujui Penuh --}}
        <div class="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-purple-100 dark:border-purple-900/50 shadow-sm flex items-center justify-between transition-colors">
            <div>
                <p class="text-[11px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider">CAR Selesai / Terbit</p>
                <h3 class="text-2xl font-black text-purple-700 dark:text-purple-400 mt-1">
                    {{ number_format($metrics['approved']) }}
                </h3>
                <p class="text-[11px] text-purple-600/80 dark:text-purple-400/80 mt-0.5">{{ $metrics['rejected'] }} Ditolak</p>
            </div>
    ...
    {{-- TABEL DATA TRANSAKSI CAR STANDAR INDUSTRI --}}
        ...
        <div class="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-sm font-bold text-slate-800 dark:text-slate-100">
                    Daftar Log Riwayat Cash Advance ({{ $daftarCar->total() }} Data Ditemukan)
                </h2>
            </div>
            <span class="text-xs text-slate-400">Halaman {{ $daftarCar->currentPage() }} dari {{ $daftarCar->lastPage() }}</span>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse" id="tableCar">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/60 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider border-b border-slate-100 dark:border-slate-700 select-none">
                        <th class="px-5 py-3.5">Dokumen & Tanggal</th>
                        <th class="px-5 py-3.5">Pemohon & Rekening</th>
                        <th class="px-5 py-3.5">Keperluan / Alasan</th>
                        <th class="px-5 py-3.5">Rincian Barang & Estimasi</th>
                        <th class="px-5 py-3.5">Total Pengajuan</th>
                        <th class="px-5 py-3.5">Status Approval</th>
                        <th class="px-5 py-3.5 text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-slate-700 dark:text-slate-300 text-xs" id="tableBodyCar">
                    @forelse($daftarCar as $car)
                        @php
                            $karyawan = $car->user;
                            $details = $car->details ?? collect();
                            $totalBiaya = $details->sum('total_harga');
                            $itemsCount = $details->count();
                            $statusTahap1 = $car->status_tahap_1;
                            $statusTahap2 = $car->status_tahap_2;
                            $statusAkhir = $car->status_akhir;
                            $nomorDoc = $car->nomor_car ?: ('CAR-' . sprintf('%04d', $car->id));

                            $detailData = $details->map(function($d) {
                                return [
                                    'nama' => $d->nama_barang,
                                    'jumlah' => (float)$d->jumlah,
                                    'satuan' => $d->satuan ?: 'pcs',
                                    'estimasi_harga' => (float)$d->estimasi_harga,
                                    'ongkir' => (float)$d->ongkir,
                                    'total_harga' => (float)$d->total_harga,
                                    'dokumen_nota' => $d->dokumen_nota_or_proposal ? asset('storage/' . $d->dokumen_nota_or_proposal) : null,
                                ];
                            });
                        @endphp
                        <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition-colors row-item"
                            data-search="{{ strtolower($nomorDoc) }} {{ strtolower($karyawan->name ?? '') }} {{ strtolower($karyawan->nip ?? '') }} {{ strtolower($car->receiving_account ?? '') }} {{ strtolower($car->alasan_pembelian ?? '') }} {{ strtolower($details->pluck('nama_barang')->implode(' ')) }}">
                            ...
                            <td class="px-5 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1">
                                        ...
                                        {{ $car->created_at ? $car->created_at->translatedFormat('d M Y') : '-' }}
                                    </span>
                                </div>
                            </td>

                            {{-- 2. Pemohon, Jabatan, Rekening & Stasiun --}}
                            <td class="px-5 py-4">
                                <div class="flex items-center space-x-3">
                                    <div class="w-9 h-9 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-bold text-xs shadow-sm overflow-hidden shrink-0">
                                    ...
                                    <div class="flex flex-col min-w-0">
                                        ...
                                        <span class="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-0.5">
                                            <i class="fa-solid fa-credit-card text-[9px] text-slate-400"></i>
                                            Rek: {{ $car->receiving_account ?: 'Pencairan Kasir' }}
                                        </span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 max-w-[200px]">
                                <p class="text-xs text-slate-700 dark:text-slate-300 font-medium line-clamp-2" title="{{ $car->alasan_pembelian ?: $car->note_explanation }}">
                                    {{ $car->alasan_pembelian ?: ($car->note_explanation ?: '-') }}
                                </p>
                            </td>

                            {{-- 4. Ringkasan Multi-Item --}}
                            <td class="px-5 py-4 max-w-[220px]">
                                <div class="flex flex-col gap-1">
                                    <span class="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1">
                                        <i class="fa-solid fa-boxes-stacked text-sky-500 text-[10px]"></i>
                                        {{ $itemsCount }} Macam Item
                                    </span>
                                    <div class="flex flex-wrap gap-1 mt-0.5">
                                        @foreach($details->take(2) as $det)
                                            <span class="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded text-[10px] truncate max-w-[150px]">
                                                {{ $det->nama_barang }} ({{ (float)$det->jumlah }} {{ $det->satuan }})
                                            </span>
                                        @endforeach
                                        @if($itemsCount > 2)
                            ...
                            {{-- 5. Total Estimasi Biaya (IDR) --}}
                            <td class="px-5 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span class="font-bold text-emerald-700 dark:text-emerald-400 text-sm">
                                        Rp {{ number_format($totalBiaya, 0, ',', '.') }}
                                    </span>
                                    <span class="text-[10px] text-slate-400">Total Anggaran Kasbon</span>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 whitespace-nowrap">
                                <div class="flex flex-col gap-1">
                                    ...
                                    {{-- Tahap 2: Manager L1/2 --}}
                                    <div class="flex items-center gap-1.5 text-[11px]">
                                        <span class="w-4 text-slate-400 font-mono text-[10px]">L1:</span>
                                        @if($statusTahap2 === 'approved')
                                            <span class="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                                                <i class="fa-solid fa-circle-check text-[10px]"></i> Disetujui Mgr
                                            </span>
                                        @elseif($statusTahap2 === 'rejected')
                                            <span class="text-rose-600 dark:text-rose-400 font-semibold flex items-center gap-1">
                                                <i class="fa-solid fa-circle-xmark text-[10px]"></i> Ditolak Mgr
                                            </span>
                            ...
                            <td class="px-5 py-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- Tombol Detail Multi-Item --}}
                                    <button type="button" onclick="openDetailCarModal({{ json_encode([
                                        'id' => $car->id,
                                        'nomor' => $nomorDoc,
                                        'nama' => $karyawan->name ?? '-',
                                        'nip' => $karyawan->nip ?? '-',
                                        ...
                                        'tanggal' => $car->created_at ? $car->created_at->translatedFormat('l, d F Y') : '-',
                                        'rekening' => $car->receiving_account ?: 'Pencairan Kasir',
                                        'alasan' => $car->alasan_pembelian ?: ($car->note_explanation ?: '-'),
                                        'total_biaya' => 'Rp ' . number_format($totalBiaya, 0, ',', '.'),
                                        'spv_status' => $statusTahap1,
                                        'spv_name' => $car->approverTahap1->name ?? '-',
                                        'mgr_status' => $statusTahap2,
                                        'mgr_name' => $car->approverTahap2->name ?? '-',
                                        'status_akhir' => $statusAkhir,
                                        'catatan_penolakan' => $car->catatan_penolakan ?: '-',
                                        'items' => $detailData,
                                        'cetak_url' => route('car.print', $car->id)
                                    ]) }})"
                                            ...
                                            title="Rincian Item Belanja">
                                        <i class="fa-solid fa-list-check text-xs"></i>
                                    </button>

                                    {{-- Tombol Cetak Dokumen PDF CAR --}}
                                    @if($statusAkhir === 'approved')
                                        <a href="{{ route('car.print', $car->id) }}" target="_blank"
                                           ...
                                           title="Cetak Formulir CAR Resmi (PDF)">
                                    ...
                                    @else
                                        <button type="button" disabled
                                                ...
                                                title="Formulir hanya dapat dicetak setelah disetujui penuh">
                                            <i class="fa-solid fa-print text-xs"></i>
                                        </button>
                                    @endif
                                </div>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 dark:text-slate-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="font-bold text-slate-600 dark:text-slate-300 text-sm">Belum Ada Riwayat CAR</p>
                                    <p class="text-xs text-slate-400 mt-1">Tidak ditemukan pengajuan Cash Advance yang sesuai kriteria filter.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
{{-- ========================================================================= --}}
{{--                  MODAL DETAIL MULTI-ITEM CAR LENGKAP                     --}}
{{-- ========================================================================= --}}
<div id="modalDetailCar" class="fixed inset-0 z-50 items-center justify-center hidden">
    <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onclick="closeDetailCarModal()"></div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl p-6 relative z-10 transform transition-all m-4 max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-700">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700">
            <div class="flex items-center gap-2.5">
                <span class="w-9 h-9 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-base border border-emerald-100 dark:border-emerald-800 shrink-0">
                    <i class="fa-solid fa-receipt"></i>
                </span>
                <div>
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 text-base" id="modalCarNomor">Detail Cash Advance Request</h3>
                    <p class="text-xs text-slate-400">Rincian nota barang, anggaran operasional, dan riwayat approval</p>
                </div>
            </div>
            <button type="button" onclick="closeDetailCarModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer transition-colors">
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700/60">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-slate-700 dark:text-slate-300">
                    <div>
                        ...
                        <span class="font-bold text-slate-800 dark:text-slate-100" id="modalCarNama">-</span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px]">NIP</span>
                        <span class="font-mono font-semibold" id="modalCarNip">-</span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px]">Stasiun / Pos</span>
                        <span class="font-semibold" id="modalCarStation">-</span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px]">Rekening Pencairan</span>
                        <span class="font-semibold text-emerald-600 dark:text-emerald-400" id="modalCarRekening">-</span>
                    </div>
                </div>

                <div class="mt-3 pt-3 border-t border-slate-200/60 dark:border-slate-700/60">
                    <span class="text-slate-400 block text-[10px] mb-0.5">Keperluan / Alasan Pembelian:</span>
                    <p class="text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs" id="modalCarAlasan">-</p>
                </div>
            </div>

            {{-- Tabel Multi-Item Barang --}}
            <div>
                <div class="flex items-center justify-between mb-2">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Rincian Item Barang / Kasbon</h4>
                    <span class="text-xs font-bold text-emerald-600 dark:text-emerald-400" id="modalCarTotalBiaya">-</span>
                </div>
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="bg-slate-100 dark:bg-slate-900/80 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200 dark:border-slate-700">
                                ...
                                <th class="px-3 py-2.5 text-right">Ongkir</th>
                                <th class="px-3 py-2.5 text-right">Total Harga</th>
                                <th class="px-3 py-2.5 text-center">Berkas Nota</th>
                            </tr>
                        </thead>
                        <tbody id="modalCarItemsBody" class="divide-y divide-slate-100 dark:divide-slate-700 text-slate-700 dark:text-slate-300">
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700/60">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div class="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                        <span class="text-[10px] text-slate-400 block">Level 3: Supervisor (Verifikasi Kebutuhan)</span>
                        <div class="font-bold text-xs mt-1" id="modalCarSpvStatus">-</div>
                        <span class="text-[10px] text-slate-500 dark:text-slate-400 block mt-0.5" id="modalCarSpvName">-</span>
                    </div>
                    <div class="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                        <span class="text-[10px] text-slate-400 block">Level 1/2: Manager (Persetujuan Dana)</span>
                        <div class="font-bold text-xs mt-1" id="modalCarMgrStatus">-</div>
                        <span class="text-[10px] text-slate-500 dark:text-slate-400 block mt-0.5" id="modalCarMgrName">-</span>
                    </div>
                </div>

                {{-- Catatan Penolakan --}}
                <div id="modalCarCatatanSection" class="mt-3 pt-3 border-t border-rose-200 dark:border-rose-900/60 hidden">
                    <span class="text-rose-500 font-bold block text-[10px] mb-0.5">Catatan Evaluasi / Penolakan:</span>
                    <p class="text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/40 p-2.5 rounded-lg border border-rose-200 dark:border-rose-900 text-xs" id="modalCarCatatan">-</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700">
            <div id="modalCarCetakContainer">
                <a id="modalCarCetakBtn" href="#" target="_blank"
                   class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm shadow-emerald-600/20 cursor-pointer">
                    <i class="fa-solid fa-print"></i> Cetak Formulir CAR (PDF)
                </a>
            </div>
            <button type="button" onclick="closeDetailCarModal()"
...
<div id="customDateModal" class="fixed inset-0 z-50 items-center justify-center hidden">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm p-6 relative z-10 m-4 border border-slate-100 dark:border-slate-700">
        ...
        <p class="text-xs text-slate-400 mb-4">Tentukan periode tanggal pengajuan CAR yang ingin ditinjau.</p>
        <div class="space-y-3">
            <div>
...
<script>
    ...
    function setPeriodePreset(val) {
        ...
        document.getElementById('filterFormCar').submit();
    }
    ...
    // EXPORT CSV CAR BERSTANDAR INDUSTRI
    function exportCsvCar() {
        const form = document.getElementById('filterFormCar');
        const formData = new FormData(form);
        const params = new URLSearchParams(formData);
        window.location.href = `{{ route('admin.record.car.export') }}?${params.toString()}`;
    }

    // LIVE TABLE SEARCH
    document.getElementById('table-search')?.addEventListener('input', function(e) {
        const term = e.target.value.toLowerCase().trim();
        const rows = document.querySelectorAll('#tableBodyCar tr.row-item');
        rows.forEach(row => {
    ...
    // MODAL DETAIL CAR
    function openDetailCarModal(data) {
        document.getElementById('modalCarNomor').textContent = 'Detail ' + data.nomor;
        document.getElementById('modalCarNama').textContent = data.nama;
        document.getElementById('modalCarNip').textContent = data.nip;
        document.getElementById('modalCarStation').textContent = data.station;
        document.getElementById('modalCarRekening').textContent = data.rekening;
        document.getElementById('modalCarAlasan').textContent = data.alasan;
        document.getElementById('modalCarTotalBiaya').textContent = 'Total: ' + data.total_biaya;

        // Render Items
        const body = document.getElementById('modalCarItemsBody');
        let html = '';
        if (data.items && data.items.length > 0) {
            data.items.forEach((it, idx) => {
                const notaHtml = it.dokumen_nota 
                    ? `<a href="${it.dokumen_nota}" target="_blank" class="px-2 py-0.5 rounded bg-sky-50 dark:bg-sky-950 text-sky-600 dark:text-sky-400 font-bold hover:underline inline-flex items-center gap-1"><i class="fa-solid fa-paperclip text-[10px]"></i> Nota</a>`
                ...
                html += `
                    <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-700/40">
                        ...
                        <td class="px-3 py-2 text-center">${it.jumlah} ${it.satuan}</td>
                        <td class="px-3 py-2 text-right font-mono">Rp ${Number(it.estimasi_harga).toLocaleString('id-ID')}</td>
                        <td class="px-3 py-2 text-right font-mono">Rp ${Number(it.ongkir).toLocaleString('id-ID')}</td>
                        <td class="px-3 py-2 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">Rp ${Number(it.total_harga).toLocaleString('id-ID')}</td>
                        <td class="px-3 py-2 text-center">${notaHtml}</td>
                    </tr>
                `;
            });
        } else {
            html = `<tr><td colspan="7" class="px-3 py-4 text-center text-slate-400 italic">Tidak ada item barang terlampir.</td></tr>`;
        }
        body.innerHTML = html;

        // SPV
        const spvEl = document.getElementById('modalCarSpvStatus');
        ...
        document.getElementById('modalCarSpvName').textContent = 'Oleh: ' + data.spv_name;

        // Mgr
        const mgrEl = document.getElementById('modalCarMgrStatus');
        ...
        document.getElementById('modalCarMgrName').textContent = 'Oleh: ' + data.mgr_name;

        // Catatan
        const catSection = document.getElementById('modalCarCatatanSection');
        if (data.status_akhir === 'rejected' && data.catatan_penolakan !== '-') {
            catSection.classList.remove('hidden');
            document.getElementById('modalCarCatatan').textContent = data.catatan_penolakan;
        } else {
            catSection.classList.add('hidden');
        }

        // Cetak
        const cetakContainer = document.getElementById('modalCarCetakContainer');
        if (data.status_akhir === 'approved') {
            cetakContainer.classList.remove('hidden');
            document.getElementById('modalCarCetakBtn').href = data.cetak_url;
        } else {
            cetakContainer.classList.add('hidden');
        }

        const modal = document.getElementById('modalDetailCar');
        modal.classList.remove('hidden');
        modal.classList.add('flex');
    }

    function closeDetailCarModal() {
        const modal = document.getElementById('modalDetailCar');
        modal.classList.add('hidden');
        modal.classList.remove('flex');
    }
