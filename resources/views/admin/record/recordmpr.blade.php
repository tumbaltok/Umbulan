        ...
        <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col gap-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center space-x-3">
                    <div class="w-12 h-12 rounded-2xl bg-sky-600/10 dark:bg-sky-950/50 text-sky-600 dark:text-sky-400 flex items-center justify-center font-bold text-xl shrink-0 border border-sky-100 dark:border-sky-800">
                        <i class="fa-solid fa-boxes-packing"></i>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">Record Material Purchase Request (MPR)</h2>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Daftar log data seluruh staf yang mengajukan pembelian material/barang.</p>
                    </div>
                </div>

                {{-- Tombol Export Data --}}
                <div class="w-full md:w-auto flex items-center justify-end">
                    <button type="button" onclick="exportExcel()" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition-all shadow-sm shadow-emerald-600/20 flex items-center gap-2 active:scale-95 cursor-pointer">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 pt-2">
                ...
                <div class="relative w-full md:w-96">
                    ...
                    <input type="text" id="table-search" placeholder="Cari pemohon, nomor MPR, atau barang..." class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-700 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all shadow-sm">
                </div>
                ...
                <form action="{{ route('admin.record.mpr') }}" method="GET" id="form-filter" class="w-full md:w-auto flex flex-wrap items-center justify-end gap-2 m-0">
                    ...
                    <div class="w-full sm:w-36">
                        <select name="bulan" id="filter_bulan" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 shadow-sm cursor-pointer">
                    ...
                    <div class="w-full sm:w-28">
                        <select name="tahun" id="filter_tahun" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 shadow-sm cursor-pointer">
            ...
            <table class="w-full text-left border-collapse" id="mpr-table">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-900/60 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider border-b border-slate-100 dark:border-slate-700 select-none">
                        ...
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100/70 dark:hover:bg-slate-800 hover:text-slate-600 transition-colors group" onclick="sortTable(2)">
                            <div class="flex items-center gap-1.5">
                                Detail Material
                                <i class="fa-solid fa-sort text-[10px] text-slate-300 group-hover:text-slate-400 transition-colors"></i>
                            </div>
                        </th>
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100/70 dark:hover:bg-slate-800 hover:text-slate-600 transition-colors group" onclick="sortTable(3)">
                            <div class="flex items-center gap-1.5">
                                Estimasi Biaya
                                <i class="fa-solid fa-sort text-[10px] text-slate-300 group-hover:text-slate-400 transition-colors"></i>
                            </div>
                        </th>
                        <th class="px-6 py-4 text-center cursor-pointer hover:bg-slate-100/70 dark:hover:bg-slate-800 hover:text-slate-600 transition-colors group" onclick="sortTable(4)">
                        ...
                        <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition-colors table-row-item">
                            ...
                            <td class="px-6 py-4 font-medium text-slate-900 dark:text-slate-100 data-name" data-value="{{ strtolower($karyawan->name ?? '') }} {{ strtolower($karyawan->nip ?? '') }}">
                                <div class="flex items-center space-x-3 btn-detail-karyawan cursor-pointer group" data-id="{{ $karyawan->id ?? '' }}">
                                    <div class="w-10 h-10 rounded-xl bg-sky-600 text-white flex items-center justify-center font-bold text-sm shadow-md shadow-sky-600/20 overflow-hidden border border-white dark:border-slate-700 shrink-0">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-slate-800 dark:text-slate-100 font-semibold text-sm group-hover:text-sky-600 dark:group-hover:text-sky-400 group-hover:underline transition-colors">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-1 text-xs text-slate-400 mt-0.5">
                                            ...
                                            <span class="text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-950/50 px-2 py-0.5 rounded-md text-[10px] font-semibold border border-sky-100 dark:border-sky-800">
                            ...
                            <td class="px-6 py-4 data-nomor" data-value="{{ strtolower($mpr->nomor_mpr ?? '') }}">
                                <div class="flex flex-col">
                                    <span class="text-sky-600 dark:text-sky-400 font-bold text-xs bg-sky-50 dark:bg-sky-950/50 px-2.5 py-1 rounded-lg border border-sky-100 dark:border-sky-800 inline-block w-fit">
                                        {{ $mpr->nomor_mpr }}
                                    </span>
                            ...
                            <td class="px-6 py-4 data-barang" data-value="{{ strtolower($mpr->items->pluck('nama_barang')->implode(' ')) }} {{ strtolower($mpr->keperluan_urgensi ?? '') }}">
                                <div class="flex flex-col gap-1.5 max-w-xs">
                                    <div class="bg-slate-100/80 dark:bg-slate-700/50 p-2 rounded-xl border border-slate-200/60 dark:border-slate-600 mb-1">
                                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Urgensi Keperluan:</span>
                                        <p class="text-xs text-slate-700 dark:text-slate-300 font-medium line-clamp-2" title="{{ $mpr->keperluan_urgensi }}">
                                    ...
                                    <div class="space-y-1">
                                        @foreach($mpr->items as $item)
                                            <div class="flex items-center justify-between text-xs bg-white dark:bg-slate-800/80 px-2.5 py-1 rounded-lg border border-slate-100 dark:border-slate-700 shadow-2xs">
                                                <span class="font-semibold text-slate-800 dark:text-slate-200 truncate">{{ $item->nama_barang }}</span>
                                                <span class="text-slate-500 dark:text-slate-300 font-bold ml-2 shrink-0 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-[10px]">{{ $item->jumlah }} {{ $item->satuan }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 text-center data-status" data-value="{{ strtolower($mpr->status_akhir ?? 'pending') }}">
                                <div class="flex flex-col items-center gap-2">
                                    @if($mpr->status_akhir === 'approved')
                                        <span class="px-3 py-1 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-bold uppercase shadow-2xs flex items-center gap-1.5">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                            Disetujui
                                        </span>
                                    @elseif($mpr->status_akhir === 'rejected')
                                        <span class="px-3 py-1 bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 rounded-xl text-xs font-bold uppercase shadow-2xs flex items-center gap-1.5">
                                            <span class="w-1.5 h-1.5 rounded-full bg-rose-500"></span>
                                            Ditolak
                                        </span>
                                    @else
                                        <span class="px-3 py-1 bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800 rounded-xl text-xs font-bold uppercase shadow-2xs flex items-center gap-1.5">
                                    ...
                                    @if($mpr->dokumen_pendukung)
                                        <a href="{{ asset('storage/' . $mpr->dokumen_pendukung) }}" target="_blank" class="inline-flex items-center gap-1 text-[11px] text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 font-bold bg-sky-50 dark:bg-sky-950/50 px-2 py-1 rounded-lg border border-sky-100 dark:border-sky-800 transition-colors">
...
<div id="detailKaryawanModal" class="fixed inset-0 z-50 items-center justify-center hidden">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg p-6 relative z-10 transform transition-all m-4 max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-700">
        <div class="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">
            <h3 class="font-bold text-slate-800 dark:text-slate-100 text-base flex items-center gap-2">
                <i class="fa-solid fa-id-card text-sky-500 dark:text-sky-400"></i> Detail Lengkap Karyawan
            </h3>
            <button type="button" id="closeDetailModalBtn" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors cursor-pointer">
        ...
        <div id="modalDataContent" class="hidden space-y-6">
            <div class="flex flex-col items-center justify-center text-center">
                <div id="detail_photo_container" class="w-20 h-20 rounded-2xl bg-sky-600 text-white flex items-center justify-center font-bold text-2xl shadow-lg overflow-hidden mb-3 border-2 border-white dark:border-slate-700 ring-4 ring-sky-50 dark:ring-sky-950"></div>
                <h4 id="detail_name" class="font-bold text-lg text-slate-800 dark:text-slate-100"></h4>
                <p id="detail_role" class="text-xs font-semibold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/60 px-3 py-1 rounded-full mt-1 border border-sky-100 dark:border-sky-800"></p>
            </div>
        ...
        <div class="flex items-center mt-6 justify-end border-t border-slate-100 dark:border-slate-700 pt-4">
            <button type="button" id="closeDetailModalBtn2" class="px-5 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-200 text-sm font-semibold rounded-xl transition-colors cursor-pointer">
                 Tutup
            </button>
        </div>
    </div>
