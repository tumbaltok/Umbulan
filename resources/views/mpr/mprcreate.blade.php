...
<div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm p-4 sm:p-6 max-w-5xl mx-auto m-2 sm:m-6 transition-colors">
    <div class="flex items-center space-x-3 mb-6">
        ...
        <div>
            <h2 class="text-lg sm:text-xl font-bold text-slate-800 dark:text-slate-100">Form Pengajuan MPR</h2>
    ...
    <form id="formMpr" action="{{ route('mpr.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-slate-50/50 dark:bg-slate-900/40 p-4 sm:p-5 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {{-- Nomor MPR (Readonly) --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Nomor MPR (Otomatis)</label>
                ...
                {{-- Priority --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Priority / Urgensi <span class="text-rose-500">*</span></label>
                    <select name="priority" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 cursor-pointer">
                ...
                {{-- Department --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Department <span class="text-rose-500">*</span></label>
                    <input type="text" name="department" value="{{ old('department', $defaultDepartment) }}" required
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Contoh: Operation">
                </div>

                {{-- Delivery Point --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Delivery Point <span class="text-rose-500">*</span></label>
                    <input type="text" name="delivery_point" value="{{ old('delivery_point', $defaultDeliveryPoint) }}" required
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Contoh: Site Umbulan">
                </div>

                {{-- Latest MPR Issued Date (Opsional) --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Latest MPR Issued Date (Opsional)</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Tanggal Pengajuan</label>
                    <input type="text" value="{{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}" readonly
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100/70 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-sm cursor-not-allowed">
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-50/50 dark:bg-slate-900/40 p-4 sm:p-5 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Rincian Catatan / Urgensi Lapangan <span class="text-rose-500">*</span></label>
                    <textarea name="keperluan_urgensi" rows="4" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-sm leading-relaxed" placeholder="- Mohon izin mengajukan pengadaan material untuk keperluan operasional&#10;- Kondisi di ruang instalasi hari ini : sisa stok kritis&#10;- Mohon segera diproses...">{{ old('keperluan_urgensi') }}</textarea>
                ...
                <div class="md:col-span-1">
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Dokumen Pendukung Utama (Opsional)</label>
                    <input type="file" id="dokumen_pendukung" name="dokumen_pendukung" class="w-full text-xs text-slate-500 dark:text-slate-400 file:mr-4 file:py-2.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-sky-50 dark:file:bg-sky-950/50 file:text-sky-700 dark:file:text-sky-300 hover:file:bg-sky-100 cursor-pointer">
                    <p class="text-[10px] text-slate-400 mt-1">* Format: PDF, JPG, JPEG, PNG (Maks 2MB)</p>

                    {{-- Container Preview untuk Dokumen --}}
                    <div id="preview-container" class="preview-container hidden mt-2 p-3 bg-slate-50 dark:bg-slate-900/60 border border-dashed border-slate-200 dark:border-slate-700 rounded-xl max-w-full">
                        <div class="flex items-center space-x-3 mb-2">
                            <span id="label-tipe-file" class="p-1.5 bg-sky-50 dark:bg-sky-950/50 text-sky-600 dark:text-sky-400 rounded-lg text-xs font-semibold uppercase tracking-wider">File</span>
                            <span id="nama-file-preview" class="text-xs text-slate-600 dark:text-slate-300 truncate font-medium">nama_file.jpg</span>
                        </div>
                ...
                <div class="baris-item bg-white dark:bg-slate-800/90 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-700 relative shadow-xs space-y-3">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                        ...
                        <div class="md:col-span-5">
                            ...
                            <input type="text" name="items[0][nama_barang]" required class="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Contoh: Refill tabung klorin site umbulan">
                        </div>

                        {{-- Quantity & Satuan & Est. Harga --}}
                        <div class="grid grid-cols-3 gap-2 md:col-span-5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Quantity <span class="text-rose-500">*</span></label>
                                <input type="number" name="items[0][jumlah]" required min="1" class="input-jumlah w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-center font-bold" placeholder="1">
                            </div>
                            <div>
                                ...
                                <input type="text" name="items[0][satuan]" required list="satuan-list" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-center" placeholder="Tabung">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Est. Harga (Rp)</label>
                                <input type="number" name="items[0][estimasi_harga]" min="0" class="input-harga w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="0">
                            </div>
                        </div>

                        {{-- Subtotal & Tombol Hapus --}}
                        <div class="md:col-span-2 flex items-center justify-between md:justify-end gap-3 pt-2 md:pt-0 border-t border-slate-50 dark:border-slate-700 md:border-none">
                    ...
                    <div class="border-t border-slate-100 dark:border-slate-700 pt-3">
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Description / Specification / Part number</label>
                        <input type="text" name="items[0][keterangan_item]" class="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-xs focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Contoh: Klorin Inti, berat tabung @900 kg / P/N: CL-900">
                    </div>
                </div>
            </div>
        ...
        <div class="p-4 bg-slate-900 dark:bg-slate-950 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-white shadow-lg shadow-slate-900/10">
            <div>
    ...
    function hitungAkumulasi() {
        ...
        semuaBaris.forEach(baris => {
            ...
            if (semuaBaris.length === 1) {
                btn.disabled = true;
                btn.className = "btn-hapus-item text-slate-300 dark:text-slate-600 cursor-not-allowed p-2 rounded-lg md:mt-4";
            } else {
                btn.disabled = false;
    ...
    btnTambahItem.addEventListener('click', function() {
        ...
        barisBaru.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                <div class="md:col-span-5">
                    <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Item / Nama Barang <span class="text-rose-500">*</span></label>
                    <input type="text" name="items[${itemIndex}][nama_barang]" required class="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Nama barang">
                </div>

                <div class="grid grid-cols-3 gap-2 md:col-span-5">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Quantity <span class="text-rose-500">*</span></label>
                        <input type="number" name="items[${itemIndex}][jumlah]" required min="1" class="input-jumlah w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-center font-bold" placeholder="1">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Satuan <span class="text-rose-500">*</span></label>
                        <input type="text" name="items[${itemIndex}][satuan]" required list="satuan-list" class="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-center" placeholder="Pcs">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Est. Harga (Rp)</label>
                        <input type="number" name="items[${itemIndex}][estimasi_harga]" min="0" class="input-harga w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="0">
                    </div>
                </div>
            ...
            <div class="border-t border-slate-100 dark:border-slate-700 pt-3">
                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Description / Specification / Part number</label>
                <input type="text" name="items[${itemIndex}][keterangan_item]" class="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 text-xs focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500" placeholder="Keterangan spesifikasi barang">
            </div>
        `;
