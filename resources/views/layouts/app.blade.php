...
<head>
    ...
    <title>Dashboard - Sistem Informasi Cuti Karyawan</title>
    ...
    <aside id="sidebarApp" class="w-64 bg-slate-900 text-slate-300 flex flex-col h-screen justify-between border-r border-slate-800 shrink-0 transition-all duration-300 z-30 fixed md:relative -translate-x-full md:translate-x-0">
        ...
        <div>
            ...
            <nav class="p-4 space-y-1.5">
                ...
                <a href="/dashboard" class="flex items-center space-x-3 bg-sky-600 text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all shadow-lg shadow-sky-900/20">
                ...
                <a href="/cuti/ajukan" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
                    <i class="fa-solid fa-paper-plane text-base w-5"></i>
                    <span>Ajukan Cuti Baru</span>
                </a>

                <a href="/cuti/riwayat" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
                    <i class="fa-solid fa-clock-rotate-left text-base w-5"></i>
                    <span>Riwayat Pengajuan</span>
                </a>

                @if(Auth::user()->role_id != 4)
                    <a href="/admin/persetujuan" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
                ...
                <a href="/admin/karyawan" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
                    <i class="fa-solid fa-users text-base w-5"></i>
                    <span>Daftar Karyawan</span>
                </a>

                <a href="admin/stations" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
                ...
                <a href="{{ route('account.index') }}" class="flex items-center space-x-3 hover:bg-slate-800 hover:text-white px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-slate-400">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <header class="bg-white border-b border-slate-100 px-6 py-4 flex justify-between items-center sticky top-0 z-20">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Selamat Datang,</p>
                    <h1 class="text-lg font-bold text-slate-800">{{ Auth::user()->name }} ({{ Auth::user()->job_title }})</h1>
                </div>
            </div>

            <div class="flex items-center space-x-4">
                <div class="text-right hidden sm:block">
                    <p class="text-sm font-bold text-slate-700">{{ Auth::user()->station->name}}</p>
                    <p class="text-[11px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md font-semibold inline-block mt-0.5">Aktif</p>
                </div>
                <div class="w-10 h-10 rounded-xl bg-sky-600 text-white flex items-center justify-center font-bold shadow-md shadow-sky-100">
                    MR
                </div>
            </div>
        </header>
